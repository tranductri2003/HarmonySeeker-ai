...
parser.add_argument(
    ...
    default=22050,
    type=int,
    help="Sample rate of songs that the original model was trained on.",
)
parser.add_argument(
    "--hop_length_original",
    default=512,
    type=int,
    help="Hop length of songs that the original model was trained on.",
)
parser.add_argument(
    "--sample_rate_transposed",
    default=22050,
    type=int,
    help="Sample rate of songs that the transposed model was trained on.",
)
parser.add_argument(
    "--hop_length_transposed",
    default=512,
    type=int,
    help="Hop length of songs that the original transposed was trained on.",
)
parser.add_argument(
    "--window_size",
    default=5,
    type=int,
    help="In case that the MLP model is used, the size of the spectrogram window used as a features.",
)
parser.add_argument(
    "--n_frames",
    default=1000,
    type=int,
    help="In case that the CRNN model is used, the length of the chord sequence that is the model trained on.",
)
parser.add_argument(
    "--skip_coef",
    default=22,
    type=int,
    help="In case that the MLP model is used, the coeficient that next spectrogram index is multiplied by when creating the window.",
)
parser.add_argument(
    "--original_model_path",
...
def main(args, waveform, sample_rate):
    ...
    C_transposed_crnn.load(
        args.transposed_model_path, args.transposed_preprocessor_path
    )
    ...
    resampled_waveform = librosa.resample(
        waveform, args.sample_rate_original, args.sample_rate_transposed
    )  # In case that the transposed model has different sample_rate
    ...
    transposed_chord_prediction = (
        C_transposed_crnn.predict(x_transposed).argmax(axis=2).flatten()
    )
    ...
    original_chord_sequence = DataPreprocessor.transpose(
        chord_sequence=chord_sequence, from_key="C", to_key=key
    )
    ...
    json_out["Key"] = key
    json_out["BPM"] = bpm
    json_out["ChordSequence"] = DataPreprocessor.chord_indices_to_notations(
